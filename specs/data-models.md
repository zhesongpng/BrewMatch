...
| `source_tier`                  | enum   | NO       | champion \| barista \| enthusiast | Source credibility tier. Defaults to `barista` when absent (legacy records). `champion` is reserved (strict) for titled world-championship winners. Trust badge in UI + near-tie breaker in retrieval (`recipe-retrieval.md` §5.3a). |
